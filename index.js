...
client.on('guildMemberAdd', async (member) => {
  const channelId = '1352033137856876574'; // استبدل برقم هوية القناة الخاصة بك
...
client.on('guildMemberAdd', async (member) => {
  // إرسال رسالة الترحيب
  const welcomeChannel = member.guild.channels.cache.get('1352033137856876574');
...
client.on("messageCreate", async (message) => {
  ...
  if (message.channel.id !== "1352033137856876574") return;
